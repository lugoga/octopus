...
```{r}
morphometric = readxl::read_excel("E:/Data Manipulation/mafia_kilwa_octopus_mapping/MorphometricNew.xlsx")

morphometric = morphometric %>% filter(Weight < 6 & DML < 25 & Kijiji != "JOJO") %>% 
  mutate(Kijiji = replace(Kijiji, Kijiji == "SOMANGA KUSINI", "Songosongo"),
         Kijiji = replace(Kijiji, Kijiji == "BWEJUU", "Bwejuu"),
         Kijiji = replace(Kijiji, Kijiji == "JIBONDO", "Jibondo"))

ggplot(data = morphometric, 
       aes(x = DML, y = Weight)) + 
  geom_point(col = "gray50", size = .9) + 
  geom_smooth(fill = "pink", alpha = 1, method = "loess", col = "red", size = 1) + 
  theme(axis.title = element_text(size = 12),
        axis.text = element_text(size = 11, colour = "black"),
        legend.position = "none",
        panel.background = element_rect(colour = "black"),
        panel.grid.minor = element_line(colour = NA))+
  labs(x = "DML (cm)", y = "Body weight (Kg)") +
  # scale_x_continuous(breaks = seq(6,25,3)) +
  # scale_y_continuous(breaks = seq(0.3,5,.75))+ 
  facet_wrap(~Kijiji, nrow = 1, scales = "fixed") 
...
ggplot(data = morphometric, aes(x = Weight, fill = Kijiji)) + geom_histogram()

```

```{r}
morphometric.group =  morphometric %>% 
  mutate(group = cut(x = Weight, breaks = seq(0,7,.5), labels = seq(0.5,7,.5))) %>% 
  group_by(group, Kijiji) %>% 
  summarise(n = n())

morphometric.group.perc = morphometric.group %>% 
  ungroup%>% filter(Kijiji == "Bwejuu") %>% mutate(perc = n/sum(n)*100) %>%
  bind_rows(morphometric.group %>% ungroup%>% filter(Kijiji == "Jibondo") %>% mutate(perc = n/sum(n)*100),
            morphometric.group %>% ungroup%>% filter(Kijiji == "Somanga") %>% mutate(perc = n/sum(n)*100),
            morphometric.group %>% ungroup%>% filter(Kijiji == "Songosongo") %>% mutate(perc = n/sum(n)*100))
...
```{r}
ggplot(data = morphometric.group.perc, 
       aes(x = group, y = perc)) + 
  geom_col(fill = "grey50", col = NA)  + 
  scale_x_discrete(breaks =seq(0.5,6.5,1)) + 
  scale_y_continuous(breaks = seq(5,50,10))+ theme(axis.title = element_text(size = 12),
        axis.text = element_text(size = 11, colour = "black"),
        legend.position = "none",
        panel.background = element_rect(colour = "black"),
        panel.grid.minor = element_line(colour = NA))+
  labs(x = "Weight group (kg)", y = "Percentage Composition") +
  facet_wrap(~Kijiji, nrow = 2)
```


```{r}
field = readxl::read_excel("e:/Data Manipulation/mafia_kilwa_octopus_mapping/Octopus.xlsx", sheet = 2)
...
village.stats = field.data %>% group_by(village, month) %>% 
  summarise(n = n(),
            ...
            catch.mean = mean(weight),
            catch.sd = sd(weight)/n) %>%
  ungroup() %>%
  mutate(miezi = month)
...
#            label = "Overall, Songosongo has\n higher CPUE than Somanga\nexcept for Month of January", 
#            col = "grey50" ,size = 3.8)
```


```{r}
ggplot(data = village.stats.order.miezi, aes(x = miezi, y = catch.mean, col = village))+
  geom_line(size = .5) +
  geom_point( size = 5) +
  geom_errorbar(aes(ymin = catch.mean-catch.sd, ymax = catch.mean+catch.sd), width = 0.25)+
  theme(panel.background = element_rect(colour = "black"),
        axis.text = element_text(size = 11),
        axis.title = element_text(size = 12),
        legend.text = element_text(size = 10),
  ...
  scale_x_continuous(breaks = 1:5,
                     labels = c("November", "December", "January", 
                                "February", "March")) +
  scale_color_discrete(name = "")  +
  labs(x = "", y = "Mean Total Landings (Kg)")
```



### Digitizing and other figures
...
bath = ggplot() +
  ...
  geom_sf(data = tz.ke, fill = "#E0D6C3", col = "#60421A", size = .5)  +
  geom_sf(data = mangroves %>% filter(CLASS == "Mangrove"), fill = "green", col = NA) +
  geom_sf(data = coral)+
  coord_sf(xlim = c(39.2,40.2), ylim = c(-8.6,-7.5)) +
  theme_bw()+
  theme(legend.direction = "horizontal", legend.key.width = unit(3, "lines"), 
  ...
  scale_fill_gradientn(limits = c(-2000,0),
  ...
  ggsn::scalebar(location = "bottomleft", dist = 20, x.min = 39.25, x.max = 40.15, y.min = -8.6, y.max = -7.5, dd2km = TRUE, model = "WGS84", st.dist = 0.02, st.size = 4)
bath
...
inset = ggplot() +
  ...
  coord_sf(ylim = c(-15, 1), xlim = c(38, 48))+
  theme_bw()+
...
```{r}
cowplot::ggdraw()+ 
    cowplot::draw_plot(bath, x = 0, y = 0, width = 1, height = 1, scale = 1)+
    cowplot::draw_plot(inset, x = .54, y = .01,width = .5, height = .5,scale = .75)
```
...
field = field %>% 
  mutate(Locality = replace(Locality,Locality == "Mbakare Ya Mwamba", "Mwamba Kaskazini"),
         ...
         Locality = replace(Locality,Locality == "Mwamba Wa Kati - Songo", "Luala"))
...
miamba.jibondo.cpue = ggplot()+
  ...
  theme(panel.background = element_rect(fill = "#6ECFF6", colour = "black"),
        ...
        legend.background = element_rect(colour = NA, fill = "#6ECFF6"), 
        legend.title = element_text(face = "bold"),
        legend.direction = "vertical",
        legend.key.width = unit(.85, "cm"),
        legend.key.height =  unit(.4, "cm"),
        axis.text = element_text(size = 10))+
  ggrepel::geom_text_repel(nudge_x = -0.015, nudge_y = 0.015, data = octopus.ground.names.df %>% 
                             filter(Area == "Jibondo" & 
                                      Name %in% c("Kitutia", 
                                                  "Mchanga nyumba", 
                                                  "Mto wa Fungu", 
...
miamba.bwejuu.cpue = ggplot() +
  ...
  theme(panel.background = element_rect(fill = "#6ECFF6", colour = "black"),
        ...
        legend.background = element_rect(fill = "#6ECFF6",colour = NA), 
        legend.title = element_text(face = "bold"),
        legend.direction = "vertical",
        legend.key.width = unit(.85, "cm"),
        legend.key.height =  unit(.4, "cm"))+
...
miamba.somanga.map = ggplot() +
  geom_sf(data = octopus.ground.sf %>% filter(type %in% c("reef flat", "submerged reef")), 
          aes(fill = type))+
  geom_sf(data = tz.ke, fill = "#E8D6B1", col = "#60421A", size = .5) +
  coord_sf(xlim = c(39.28, 39.60), ylim = c(-8.46, -8.26))+
  scale_fill_manual(values = c("#B2DF8A","#66C2A5"),
                    label = c("Reef flat", "Subermeged Reef"), 
                    name = "Habitat type")+
  theme_bw()+
  ...
  # annotate(geom = "text", x = 39.685,y = -7.96, label = "Mafia\nIsland")+
  labs(x = "", y = "") +
  scale_x_continuous(breaks = seq(39.28, 39.60,length.out = 4))+
  scale_y_continuous(breaks = seq(-8.46, -8.28, length.out = 4))
...
cowplot::plot_grid(miamba.somanga.map, miamba.somanga.cpue, nrow = 1, 
                   labels = c("a)", "b)"),label_x = .2, label_y = .98,
...
cowplot::plot_grid(miamba.songosongo.map, miamba.songosongo.cpue, 
                   nrow = 1, labels = c("a)", "b)"), label_x = .2, label_y = .7,
                   label_fontface = "plain", label_size = 12)
...
```{r}
section.chooni.mange = section.mafia %>% subset(stationId %in% c(12,29,30,31,32,33,34)) 


section.chooni.mange %>% plot(xtype = "longitude", ztype = "image", ylim = c(.50,5), which = c("temperature", "oxygen", "fluorescence", "map"))
```

```{r}
section.chooni.mange.list =  section.chooni.mange@data$station

section.chooni.mange.df = NULL

for (n in 1:7){
chooni.mange = section.chooni.mange.list[[n]]@data %>% 
  as.data.frame() %>% 
  mutate(lon = section.chooni.mange.list[[n]]@metadata$longitude, 
         lat = section.chooni.mange.list[[n]]@metadata$latitude ,
         station = n)

section.chooni.mange.df = section.chooni.mange.df %>% bind_rows(chooni.mange)
  
}

section.chooni.mange.df = section.chooni.mange.df%>% 
  select(lon,lat, station, pressure, temperature, salinity, oxygen, fluorescence)

```


```{r}
x = section.chooni.mange.df$lon; y = section.chooni.mange.df$pressure; z = section.chooni.mange.df$temperature

temperature.interp = interpBarnes(x = x, y = y, z = z)
Lon = temperature.interp$xg
Depth = temperature.interp$yg
Temperature = temperature.interp$zg

imagep(Lon, Depth*-1, Temperature)
