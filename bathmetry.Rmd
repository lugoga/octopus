...
ggplot() + 
  ...
  coord_sf(xlim = c(39.45,39.85), ylim = c(-8.13,-7.85))+
  geom_contour(data = bathmetry %>% filter(bathmetry > -40 & bathmetry <5), 
              aes(x = lon, y = lat, z = bathmetry), breaks = seq(-40, 0, 2), col = "grey50" )+
  geom_contour(data = bathmetry %>% filter(bathmetry > -1000 & bathmetry <5), 
              aes(x = lon, y = lat, z = bathmetry), breaks = seq(-1000, 0, 20), col = 1)+
  theme_bw()+
...
```{r}
depth = bathmetry %>% filter(bathmetry > -1000 & bathmetry <5) %>% mutate(depth = bathmetry*-1)

depth.interp = interpBarnes(x = depth$lon, y = depth$lat, z = depth$depth, xg = pretty(x = depth$lon, n = 10), yg = pretty(x = depth$lat, n = 10))

image(depth.interp$xg, depth.interp$yg, depth.interp$zg)
contour(depth.interp$xg, depth.interp$yg, depth.interp$zg, add = TRUE)
