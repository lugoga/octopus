...
ggplot() +
  ...
  geom_contour(data = bathmetry %>% filter(bathmetry > -100 & bathmetry <5), 
              ...
              breaks = c(-10,-20,-30,-40,-50), col = "grey80")+
...
```{r miamba-jibondo, fig.cap="Reefs commonly used for octopus fishery around Jibondo"}
...
```{r miamba-bwejuu, fig.cap="Reefs commonly used for octopus fishery around Bwejuu"}
...
```{r miamba-somanga, fig.cap="Reefs commonly used for octopus fishery around Somanga"}
...
```{r miamba-songosongo, fig.cap="Reefs commonly used for octopus fishery around Songosongo"}
...
ggplot() +
  ...
  scale_fill_simpsons(name = "Octopus Areas", limits = c("Island", "submerged reef", "coastal-shallow", "reef flat"))+
  ggrepel::geom_text_repel(data = octopus.ground.names.df %>% 
                             filter(Area == "Jibondo"& type %in% c("reef flat", "Island", "subemerged reef", "coastal-shallow")),
...
jibondo.bwejuu = field %>%
  ...
  select(date, month = Month, village = Kijiji, diko = Diko, locality = Locality,
         vessels = 7,engine.power = 8, 
         fisher.number = 9, fishing.ground = 11, fish.type = 13,species = 14,
         Family = 15,weight = 16, number.octopus = 17,cpue = 18) %>% 
  mutate(catch.rate = weight/fisher.number, month = month(date))%>%
...
ggplot(data = jibondo.freq,  aes(x = locality, y = percentage.fisher))+
  ...
  scale_x_discrete(limits = c("Mchanga Nyuma", "Lwala", "Mto Wa Fungu", "Kitutia") %>% rev())+
...
```{r tab12}
bwejuu.freq = jibondo.bwejuu %>% filter(village == "Bwejuu") %>% group_by(locality) %>% summarise(Fisher = n()) %>% ungroup() %>% mutate(percentage.fisher = Fisher/sum(Fisher)*100) %>% arrange(desc(percentage.fisher))
...
ggplot(data = bwejuu.freq,  aes(x = locality, y = percentage.fisher))+
  geom_col(fill = "grey50", width = .75)+
  coord_flip()+
  theme_minimal()+
  theme(axis.text = element_text(size = 11, colour = "grey50"),
        ...
        panel.grid.minor.x = element_blank())+
  labs(y = "Number of Octopus Fishers",  x = "Fishing ground") +
  scale_x_discrete(limits = c("Mnyamalile", "Mlima", "Vijambani", "Mwamba Kaskazini", "Kaule", "Rasi Ya Mwamba", "Ngusi") %>% rev())+
  scale_y_continuous(breaks = seq(10,60,10), position = "right")
```
...
field.octopus %>% 
  group_by( village) %>% 
  summarise(count = n(), cpue.bar = mean(cpue), cpue.median = median(cpue), 
            cpue.sd = sd(cpue, na.rm = TRUE)/sqrt(count)) %>% 
  filter(!is.na(cpue.sd)) %>% 
  arrange(cpue.bar) %>% select(-cpue.median) %>% kable("latex", booktabs = T) %>%
